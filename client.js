...
function getPlayerIcon(imgElement, displayName, internalPlayerName, allowNameVariations = false) {
    ...
    let match = internalPlayerName ? internalPlayerName.match(/\d+/) : null;
    let playerNumber = match ? match[0] : null;
    if (!playerNumber && window.gameState && Array.isArray(gameState.jugadoresInfo)) {
        const idx = gameState.jugadoresInfo.findIndex(p => p.name === internalPlayerName);
        if (idx !== -1) playerNumber = (idx + 1).toString();
    }
    ...
    if (srcHintRaw && typeof srcHintRaw === 'string') {
        ...
        try {
            ...
            imgElement.onerror = function() {
                // Hide broken <img> to avoid broken icon and fall back to deterministic default
                try { imgElement.style.display = 'none'; } catch (e) {}
                try {
                    const parent = imgElement.parentElement;
                    if (parent) {
                        // Apply the default jugadorX avatar as a visual fallback
    ...
    // If probing is disabled, immediately use the default jugadorX avatar.
    if (!allowNameVariations) {
        // Immediately set parent background to default to avoid empty UI
        try {
            const parent = imgElement.parentElement;
            if (parent) {
                parent.dataset.assignedSrc = defaultAvatarSrc;
        ...
        // Load into <img> but only hide after successful load
        try {
            imgElement.onload = function() {
...
function updatePlayersUI() {
    ...
    // (debug badge removed - using external overlay instead)
        ...
        if (playerData.avatar && playerData.avatar.type === 'file') {
            ...
            // If the server supplied a direct image URL or data URI, use it immediately
            console.info('Avatar processing for', playerData.name, 'avatar:', playerData.avatar);
                    const data = playerData.avatar && playerData.avatar.data;
                    const hasNonEmptyData = typeof data === 'string' && data.trim().length > 0;
                    console.info('  avatar.data for', playerData.name, 'hasNonEmptyData:', hasNonEmptyData, 'value:', data);
                    if (hasNonEmptyData) {
                        // Normalize bare filenames to assets path and forward as srcHint
                        let hint = data;
                        if (!hint.startsWith('data:') && hint.indexOf('/') === -1) {
                            hint = `/assets/icons/${hint}`;
                        }
                        try { img.dataset.srcHint = hint; } catch (e) {}
                        // Immediately set parent background to avoid blank avatar
                        try {
                            const parent = img.parentElement;
                            if (parent) {
                                parent.style.backgroundImage = `url(${hint})`;
                                parent.style.setProperty('background-image', `url(${hint})`, 'important');
                                parent.style.backgroundSize = 'cover';
                                parent.style.backgroundPosition = 'center';
                            }
                        } catch (e) {}
                        console.info('  forwarded normalized srcHint for', playerData.name, '->', img.dataset.srcHint);
                        try { getPlayerIcon(img, playerData.displayName, playerData.name, true); } catch (e) {}
                    } else {
                        // Server indicates a file-backed avatar but didn't provide data.
                        // Still try a normalized on-disk avatar path.
                        // Prefer the deterministic jugadorX form when a number exists,
                        ...
                        try {
                            const parent = img.parentElement;
                            if (parent) {
                                parent.style.backgroundImage = `url(${guessed})`;
                                parent.style.setProperty('background-image', `url(${guessed})`, 'important');
                                parent.style.backgroundSize = 'cover';
                                parent.style.backgroundPosition = 'center';
                            }
                        } catch (e) {}
                        console.info('  server requested file avatar, probing canonical file for', playerData.name, '->', guessed);
                        try { getPlayerIcon(img, playerData.displayName, playerData.name, true); } catch (e) {}
                    }
            ...
            try {
                ...
                const dataUri = playerData.avatar.data;
                if (dataUri && typeof dataUri === 'string') {
                    // Use the data URI as a srcHint so getPlayerIcon handles caching/onload consistently
                    const img = document.createElement('img');
                    img.style.width = '40px';
                    img.style.height = '40px';
                    img.style.borderRadius = '50%';
                    ...
                    // Set parent background immediately (data URI on top of SVG placeholder)
                    try {
                        avatarDiv.style.backgroundImage = `url(${dataUri}), url(${svgPlaceholder})`;
                        try { avatarDiv.style.setProperty('background-image', `url(${dataUri}), url(${svgPlaceholder})`, 'important'); } catch (e) {}
                        avatarDiv.style.backgroundSize = 'cover';
                        avatarDiv.style.backgroundPosition = 'center';
                    } catch (e) {}
    ...
    let finalDisplayName = playerNum ? `${playerData.displayName} (${playerNum})` : playerData.displayName;
        ...
        // Optional per-player avatar diagnostic (disabled by default).
        // Enable by setting `window.__avatarDebugVerbose = true` in the console.
        try {
            if (window.__avatarDebugVerbose) {
                setTimeout(() => {
                    const img = avatarDiv.querySelector('img');
                    console.info('[AVATAR DEBUG]', playerName, {
                        avatarField: playerData.avatar,
                        parentBackground: avatarDiv.style.backgroundImage,
                        parentAssigned: avatarDiv.dataset.assignedSrc || null,
                        imgSrc: img ? img.src : null,
                        imgAssigned: img ? (img.dataset && img.dataset.assignedSrc) : null,
...
window.forceReprobeAvatars = function() {
    try {
        if (window.avatarAssigned) {
            Object.keys(window.avatarAssigned).forEach(k => delete window.avatarAssigned[k]);
        }
        window.avatarAssigned = {};
        console.info('Cleared avatarAssigned cache — re-rendering player UI to re-probe avatars');
...
// createAvatarOverlay removed to clean up UI on mobile / production

window.createPointsTableNow = function() {
